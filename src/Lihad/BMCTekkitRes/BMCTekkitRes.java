...
public class BMCTekkitRes extends JavaPlugin implements Filter{
	...
	public boolean isLoggable(LogRecord record) {
		...
		if (record.getMessage() != null) {
			...
			boolean c = false;
			if (record.getLevel().getName().toLowerCase().contains("slotchanging")) {
				return false;
			} else if (a.toLowerCase().contains("slotchanging")) {
				return false;
			}
		}
		return true;
	...
	public void setPlayerInv(String name) {
		try {
			...
			if (cc != null) {
				...
				CraftPlayer ccr = (CraftPlayer) this.getServer().getPlayer(name);
